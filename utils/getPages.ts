...
export default function getPages(): PageProps[] {
    ...
    const files = fs.readdirSync('public/blog/content/pages')
    const pages = files.map((directory) => {
        const readFile = fs.readFileSync(`public/blog/content/pages/${directory}/index.en.md`, 'utf-8')
        ...
        .filter(page => process?.env?.NODE_ENV === 'production' ? !page?.params?.frontmatter?.draft : true)
    return pages
}
